...
class SkeletonEngine:
    ...
    def __init__(self, obj_body, obj_armat, data_path):
        if obj_body and obj_armat:
            if bpy.data.objects[obj_armat.name].type == 'ARMATURE':
                ...
                self.joints_indices = {}
                self.bones_z_axis = {}
                self.shared_joints_path = self.body_name[:len(self.body_name)-2]+"_joints.json"                
                self.joints_data_path = os.path.join(
                    data_path,
                    "shared_joints",
                    self.shared_joints_path)
                self.jointsDatabase = self.load_joints_database(self.joints_data_path)
                self.store_bones_z_axis()
            else:
                lab_logger.error("The object {0} is not an armature".format(obj_armat.name))
        else:
            lab_logger.error("Body or armature (or both) not found ")
    ...
    def store_armature_visibility(self, armature_object):
        lab_logger.debug("Storing the armature visibility")
    ...
    def restore_armature_visibility(self, armature_object):
        lab_logger.debug("Restoring the armature visibility")
    ...
    def force_visible_armature(self, armature_object):
        ...
        to handle it.
        """   
        lab_logger.debug("Turn the armature visibility ON")     
    ...
    def save_pose(self, data_path):
        ...
        if armat:            
            ...
            self.force_visible_armature(armat)            
            for bone in armat.pose.bones:            
                matrix_data[bone.name] = [value for value in bone.rotation_quaternion] 
            self.restore_armature_visibility(armat)
    ...
    def is_in_rest_pose(self):
        ...
        if armat:
            ...
            is_rest = True
            for bone in armat.pose.bones:
                if bone.rotation_quaternion != reset_quat:
                    is_rest = False            
            self.restore_armature_visibility(armat)
        return is_rest
