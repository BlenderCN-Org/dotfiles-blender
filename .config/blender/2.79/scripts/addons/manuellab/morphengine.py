...
class MorphingEngine:
    ...
    def __init__(self, obj, data_path):
        ...
        if obj:
            ...
            self.shared_morphs_filename = obj.name[:len(obj.name)-2]+".json"
            self.shared_bodies_path = obj.name[:len(obj.name)-2]+"_bodies"
            self.shared_measures_filename = obj.name[:len(obj.name)-2]+"_measures.json"
            self.shared_bbox_filename = obj.name[:len(obj.name)-2]+"_bbox.json"
    ...
    def load_measures_database(self, measures_path):
        if os.path.isfile(measures_path):
            ...
            try:
                ...
                lab_logger.info("Measure database {0} loaded with {1} measures".format(algorithms.simple_path(measures_path),len(self.measures_data)))
            except:
                lab_logger.error("Error decoding {0}".format(algorithms.simple_path(measures_path)))
            database_file.close()
        else:
            lab_logger.warning("Measures database not found")
    ...
    def load_bboxes_database(self, bounding_box_path):
        if os.path.isfile(bounding_box_path):
            ...
            try:
                ...
                lab_logger.error("Error decoding {0}".format(algorithms.simple_path(bounding_box_path)))
            database_file.close()
            lab_logger.info("Boundingboxes database {0} loaded in {1} secs".format(algorithms.simple_path(bounding_box_path),time.time()-time1))
        else:
    ...
    def load_vertices_database(self, vertices_path):
        if os.path.isfile(vertices_path):
            ...
            try:
                ...
                lab_logger.error("Error decoding {0}".format(algorithms.simple_path(vertices_path)))
            database_file.close()
            for vert_co in verts:
                self.base_form.append(mathutils.Vector(vert_co))
            lab_logger.info("Vertices database {0} loaded in {1} secs".format(algorithms.simple_path(vertices_path),time.time()-time1))
        else:
